...
app.get("/data/prescriptions", async (req, res) => {
    ...
    try {
        ...
        const userMedications = medications.filter(med => med.patientName === name);
...
app.put("/data/prescriptions", async (req, res) => {
    ...
    if (!uuid || !newDate) {
        return res.status(400).json({ error: "Missing 'uuid' or 'newDate' in request body" });
    }
    if (!existsSync(MED_DATA_PATH)) {
        return res.status(404).json({ error: "Medication data file not found" });
    }
    ...
    try {
        ...
        res.json({ success: true, message: "Medication updated successfully", updatedMedication: medications[index] });
    } catch (err) {
...
app.post("/data/users", async (req, res) => {
    try {
        ...
        const users = existsSync(USER_DATA_PATH) ? await readJson(USER_DATA_PATH) : [];
...
app.post("/data/prescriptions", async (req, res) => {
    try {
        ...
        const medications = existsSync(MED_DATA_PATH) ? await readJson(MED_DATA_PATH) : [];

        medications.push(newMedication);
        await writeJson(MED_DATA_PATH, medications, { spaces: 2 });

        res.status(201).json({ success: true, message: "Medication added successfully", medication: newMedication });
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
});
